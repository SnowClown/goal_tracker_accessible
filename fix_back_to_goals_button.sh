#!/bin/bash
# Adds the "Back to Goals" button to the Tasks screen's AppBar.
# This targets the file's ACTUAL current content (confirmed via cat),
# since an earlier script reported success without the change
# actually landing here.
#
...
TARGET="lib/version_a/screens/tasks_list_screen.dart"
...
if count > 1:
    raise SystemExit(f"Error: found {count} matches, expected exactly 1 — file may have unexpected duplicates.")
...
print(f"Updated {path} with 'Back to Goals' button (green arrow, size 30)")
PYEOF
